...
### 2.4 Install Native Libraries (Opus + dav1d)

The client depends on native Opus and `dav1d` libraries on Windows. The
recommended approach is **vcpkg** so Cargo can link against MSVC-compatible
`.lib` files.
...
# Install 64-bit Windows libraries
.\vcpkg.exe install opus:x64-windows dav1d:x64-windows
```

Verify that `dav1d.lib` exists:

```powershell
dir C:\src\vcpkg\installed\x64-windows\lib\dav1d.lib
```

Set the required environment variables. `VCPKG_ROOT` is used by the build to
locate both Opus and `dav1d`, and `VCPKGRS_DYNAMIC=1` matches the default
dynamic `x64-windows` triplet:
...
# Optional: set explicitly if you keep dav1d somewhere else
# DAV1D_ROOT must be the triplet root that contains lib/ and debug/
# Example:
# $env:DAV1D_ROOT="C:\src\vcpkg\installed\x64-windows"
```

To persist those variables for future shells:
...
> **Important:** Do not mix MinGW/MSYS2 builds of `dav1d` with the Rust
> `x86_64-pc-windows-msvc` target. Use an MSVC-compatible `dav1d.lib` from
> vcpkg or another MSVC build.

> **Runtime note:** The default `x64-windows` vcpkg triplet is dynamic. After a
> successful link, you may also need `dav1d.dll` next to `vp-client.exe`:
>
> ```powershell
> copy C:\src\vcpkg\installed\x64-windows\bin\dav1d.dll C:\tsod\client\target\release\
> ```
>
> If you prefer static linking, install `dav1d:x64-windows-static` and update
> your native library search path accordingly.

**Alternative:** If you have pre-built `opus.lib`/`opus.dll` or an MSVC-built
`dav1d.lib`, point your build environment at those locations directly instead of
using vcpkg.


### 2.5 Clone and Build

Before building on Windows, open **Developer PowerShell for Visual Studio** or
**Developer Command Prompt for Visual Studio** so `link.exe` and the Windows SDK
are on the PATH.

If you installed vcpkg somewhere other than `C:\src\vcpkg`, set `VCPKG_ROOT`
for the current session before building:

```powershell
$env:VCPKG_ROOT="C:\src\vcpkg"
# Optional if dav1d is in a nonstandard location:
# $env:DAV1D_ROOT="C:\path\to\dav1d\installed\x64-windows"
```
...
Install required packages, including the `dav1d` development package used by the
client's native AV1 dependency:
...
Verify that `pkg-config` can see `dav1d`:

```bash
pkg-config --libs dav1d
pkg-config --cflags dav1d
```

If those commands fail, the package is either not installed or its `.pc` file is
not on your `PKG_CONFIG_PATH`.
...
> **Other distros:** On Fedora, install `pkgconf-pkg-config` and
> `libdav1d-devel`. On Arch Linux, install `pkgconf` and `dav1d`.
...
- **Opus linking errors (Windows)**: Verify `VCPKG_ROOT` is set and
  ...
  `VCPKGRS_DYNAMIC=1`
- **`LINK : fatal error LNK1181: cannot open input file 'dav1d.lib'` (Windows)**:
  Verify `dav1d` is installed with vcpkg and that the build can find it:
  ```powershell
  cd C:\src\vcpkg
  .\vcpkg.exe install dav1d:x64-windows
  $env:VCPKG_ROOT="C:\src\vcpkg"
  cargo clean
  cargo build --release -vv
  ```
  Also confirm the file exists at
  `C:\src\vcpkg\installed\x64-windows\lib\dav1d.lib`
- **`dav1d.dll was not found` when launching `vp-client.exe` (Windows)**:
...
- **`pkg-config` cannot find `dav1d` (Linux)**: Install `libdav1d-dev`:
