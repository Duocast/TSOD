...
### 2.4 Install Opus and dav1d Libraries (Windows)

The client depends on native `opus` and `dav1d` libraries on Windows. The
recommended approach is **vcpkg**. Use a consistent install path and keep it the
same everywhere. This guide assumes:

```text
C:\src\vcpkg
```

Open **Developer PowerShell for Visual Studio** or **Developer Command Prompt
for Visual Studio**, then run:
...
# Install native libraries for 64-bit Windows
...
```powershell
dir C:\src\vcpkg\installed\x64-windows\lib\opus.lib
dir C:\src\vcpkg\installed\x64-windows\lib\dav1d.lib
dir C:\src\vcpkg\installed\x64-windows\bin\dav1d.dll
```

Set the required environment variables. Use the same path you cloned vcpkg to:

```powershell
# Persistent (User scope)
[Environment]::SetEnvironmentVariable("VCPKG_ROOT", "C:\src\vcpkg", "User")
[Environment]::SetEnvironmentVariable("DAV1D_ROOT", "C:\src\vcpkg\installed\x64-windows", "User")
...
> **Important:** Do not leave `VCPKG_ROOT` pointing at `C:\vcpkg` if the real
> install is `C:\src\vcpkg`. A stale `VCPKG_ROOT` value causes the linker to
> search the wrong directory and fail with `LNK1181: cannot open input file
> 'dav1d.lib'`.

**Alternative:** If you have pre-built `opus.lib` and `opus.dll`, set
`OPUS_LIB_DIR` to the directory containing them. For `dav1d`, the build script
expects either `DAV1D_ROOT` or `VCPKG_ROOT` to resolve to the folder containing
`lib\dav1d.lib`.
...
Verify the native libraries can be discovered:
...
> **Important:** The Linux `build.rs` path for `dav1d` uses the Rust
> `pkg-config` crate at build time. In `client/Cargo.toml`, `pkg-config = "0.3"`
> must be under `[build-dependencies]`, not `[dependencies]`.
...
If `pkg-config` cannot find `dav1d`, confirm the package is installed and the
metadata is visible:
...
- **CMake "Compatibility with CMake < 3.5 has been removed" (Windows)**: CMake
  ...
  cargo build --release
  ```
- **Opus / dav1d linking errors (Windows)**: Verify `VCPKG_ROOT` points to the
  actual vcpkg install path and that both installs completed successfully:
  `C:\src\vcpkg\vcpkg.exe install opus:x64-windows dav1d:x64-windows`
  Also verify `DAV1D_ROOT=C:\src\vcpkg\installed\x64-windows`. A stale
  `VCPKG_ROOT=C:\vcpkg` is enough to cause `LNK1181: cannot open input file
  'dav1d.lib'`.
- **Opus / dav1d linking errors (Linux)**: Install the development packages:
  `sudo apt install libopus-dev libdav1d-dev pkg-config`
- **`use of unresolved module or unlinked crate pkg_config` during Linux build**:
  Move `pkg-config = "0.3"` into `[build-dependencies]` in `client/Cargo.toml`
- **`pkg-config` cannot find `dav1d` (Linux)**: Check the package metadata:
  `pkg-config --libs dav1d`
