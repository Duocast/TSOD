...
use crate::net::video_frame::{EncodedFrame, PixelFormat, VideoFrame};
...
pub fn build_screen_encoder(codec: &str, _profile: &str) -> Result<Box<dyn VideoEncoder>> {
    match codec {
        "AV1" if cfg!(feature = "video-av1") => Ok(Box::new(av1::Av1RealtimeEncoder::new())),
    ...
    match frame.format {
        PixelFormat::Bgra => {
            let plane = frame
                .planes
                .first()
                .ok_or_else(|| anyhow!("missing BGRA source plane"))?;
            let src = &plane.data;
