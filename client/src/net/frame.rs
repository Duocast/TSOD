...
mod tests {
    ...
    fn oversized_frame_rejected() {
        assert!(validate_frame_len(MAX_CONTROL_FRAME_LEN + 1, MAX_CONTROL_FRAME_LEN).is_err());
    }

    #[test]
    fn delimited_roundtrip_bytes() {
        let msg = pb::ClientToServer {
            request_id: Some(pb::RequestId { value: 7 }),
            ..Default::default()
        };
        let mut body = BytesMut::with_capacity(msg.encoded_len());
        ...
        let decoded =
            pb::ClientToServer::decode(&frame[consumed..consumed + len as usize]).unwrap();
        assert_eq!(decoded.request_id.unwrap().value, 7);
    }
}
